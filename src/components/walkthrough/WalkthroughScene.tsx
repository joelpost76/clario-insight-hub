...
export function WalkthroughScene({ scene, isActive }: WalkthroughSceneProps) {
  ...
  useEffect(() => {
    if (isActive) {
      ...
      const timeTimer = setTimeout(
        ...
        800 + scene.bullets.length * 250 + 400
      );
  ...
  return (
    <div
      className={cn(
        "absolute inset-0 flex flex-col items-center justify-center text-center px-6 transition-all duration-700 ease-in-out",
    ...
    >
      ...
      <h2
        className={cn(
          "text-2xl md:text-3xl font-bold text-foreground mb-2 transition-all duration-700 delay-200",
      ...
      <p
        className={cn(
          "text-muted-foreground text-base md:text-lg max-w-lg mb-8 transition-all duration-700 delay-300",
          visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"
        )}
      >
        {scene.subtitle}
      ...
      {scene.timeNote && (
        <p
          className={cn(
            "text-xs md:text-sm font-medium text-primary/80 tracking-wide uppercase transition-all duration-700",
